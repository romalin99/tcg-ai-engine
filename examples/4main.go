// grule-rule-engine 进阶示例 —— 风控打分 + 仿 skyhackvip/risk_engine 决策流
//
// 演示 grule 的八个核心机制:
...
//  8. 决策流融合(第二部分): 仿 skyhackvip/risk_engine 的
//     特征(feature 类型系统)/UDF(sum/avg/min/max)/条件分流/决策树/
//     交叉决策表/决策流执行短路 —— 见 demoRiskEngineFlow 起的后半篇
//
// 运行: go run ./examples/4main.go
...
// ============================================================
// ★ 第二部分: 仿 skyhackvip/risk_engine 的决策流融合 ★
// (https://github.com/skyhackvip/risk_engine — "天网"决策引擎)
//
// repo 架构 ↔ 本演示(grule 承载)的对照表:
//
//	repo 构件                      repo 源码           本文件对应物
//	──────────────────────────────────────────────────────────────
//	特征 IFeature 类型系统          core/feature.go     FeatureSet(类型安全访问器+缺省回退)
//	UDF 内建函数 sum/avg/min/max    core/udf/*.go       UdfLib(fact 方法即 UDF)
//	决策流 DecisionFlow.Run 逐节点   core/flow.go        DecisionFlow.Run
//	决策流执行短路 NodeResult.IsBlock core/flow.go        NodeResult.IsBlock → Run 提前返回
//	规则集节点 + block_strategy     core/ruleset.go     grule 规则集节点(grlAdmission)
//	条件分流节点(顺序求值,首匹配)     core/conditional.go FlowNode.Route
//	决策树节点                      core/tree.go        GRL"叶子规则 = 根到叶全路径条件"
//	决策表/交叉决策表(决策矩阵)      core/matrix.go      GRL"单元格规则 = 行条件×列条件"
//	执行轨迹 ctx.AddTrack           core/pipeline.go    FlowCtx.Track
//
// 与 repo 的本质差异: risk_engine 自研表达式求值器(internal/operator),
// 特征/操作符/UDF 都是解释执行 yaml DSL; 本文件把"节点内的规则求值"交给
// grule(GRL 编译执行), 把"节点间的流程编排"留在 Go —— 各取所长:
// grule 不擅长流程图, Go 编排恰好补位; repo 的 DSL 灵活性换成了 GRL 的表达力。
//
// ── 短路的三个层次(由内到外, 本文件全齐) ──
//	① 规则层: salience 冲突消解 —— 每 cycle 只执行一条(第一部分已示范);
//	② 节点内: Complete() —— 阻断规则命中后本规则集剩余规则不再评估
//	   (对应 repo ruleset 串行执行遇 block 即停);
//	③ 流级:   NodeResult.IsBlock=true → DecisionFlow.Run 停止推进,
//	   下游节点(条件分流/决策树/决策表/end)连知识库实例都不会创建
//	   (对应 repo flow.go: if res.IsBlock { gotoNext = false })。
// ============================================================

// FeatureSet 特征集 —— 对应 repo core/feature.go 的 IFeature 类型系统。
// repo: 特征分 int/float/string/bool/date/array/map 七类, 类型决定可用操作符
// (数值才能 GT/LT/BETWEEN, 数组才有 IN/value-exist), Compare 前先校验;
// GetValue 返回 (value, exists), 缺失回退默认值。
// 本文件: 用"类型安全访问器"承载同一思想 —— Num/Str/Flag/StrIn 各管一类,
// 类型不符/特征缺失返回零值而不是 panic(呼应第一部分"数组越界静默跳过规则"
// 的教训: 规则悄悄失效比崩溃更隐蔽, 防护必须做在 Fact 层)。
type FeatureSet struct {
	m map[string]interface{}
}

func NewFeatureSet(m map[string]interface{}) *FeatureSet { return &FeatureSet{m: m} }

// Has 特征是否存在 —— 对应 repo 操作符 "key exist"
func (f *FeatureSet) Has(name string) bool { _, ok := f.m[name]; return ok }

// Num 数值特征(int/int64/float64 皆收) —— 对应 TypeNumFeature; 缺失/类型不符返回 0
func (f *FeatureSet) Num(name string) float64 {
	switch v := f.m[name].(type) {
	case int:
		return float64(v)
	case int64:
		return float64(v)
...
// NumOr 带默认值的数值特征 —— 对应 repo GetValue 的 (value,exists)+默认值兜底:
// 特征平台没算出来(冷启动/超时)时, 规则用兜底值继续跑而不是整条失效
func (f *FeatureSet) NumOr(name string, def float64) float64 {
	if !f.Has(name) {
		return def
	}
	return f.Num(name)
...
// Str 字符串特征 —— 对应 TypeStringFeature
func (f *FeatureSet) Str(name string) string {
	if v, ok := f.m[name].(string); ok {
		return v
	}
	return ""
}

// Flag 布尔特征 —— 对应 TypeBoolFeature
func (f *FeatureSet) Flag(name string) bool {
	if v, ok := f.m[name].(bool); ok {
		return v
	}
	return false
}

// StrIn 值是否在数组特征中 —— 对应 TypeArrayFeature 的 IN / "value exist" 操作符
func (f *FeatureSet) StrIn(arrName, val string) bool {
	arr, ok := f.m[arrName].([]string)
	if !ok {
		return false
	}
	for _, s := range arr {
...
// nums 数值数组特征(内部, 供 UDF 用) —— 对应 TypeArrayFeature
func (f *FeatureSet) nums(name string) []float64 {
	arr, _ := f.m[name].([]float64)
	return arr
}

// Dump 按 key 排序输出全部特征(演示可读性; map 直接打印顺序不稳定)
func (f *FeatureSet) Dump() string {
	...
	for _, k := range keys {
		parts = append(parts, fmt.Sprintf("%s=%v", k, f.m[k]))
	}
	return strings.Join(parts, " ")
}

// UdfLib UDF 函数库 —— 对应 repo core/udf/{sum,avg,min,max}.go 四个内建函数。
// repo: type Udf func(interface{}) (interface{}, error), 按名注册进 registry,
// 表达式解析器识别函数调用语法后查表执行;
// grule 里"表达式可调用的函数"天然就是 fact 方法 —— 注入名 Udf 即函数命名空间,
// GRL 写 Udf.Max("txn_amounts_7d") 与 repo 表达式里 max(feature) 等价。
// 空数组统一返回 0(演示从简; 生产应连同 exists 一起返回或走 NumOr 模式)。
type UdfLib struct {
	F *FeatureSet
}

// Sum 数组特征求和
func (u *UdfLib) Sum(name string) float64 {
	s := 0.0
	for _, v := range u.F.nums(name) {
		s += v
	}
	return s
}
...
// Avg 数组特征均值
func (u *UdfLib) Avg(name string) float64 {
	arr := u.F.nums(name)
	if len(arr) == 0 {
		return 0
	}
	return u.Sum(name) / float64(len(arr))
}

// Max 数组特征最大值
func (u *UdfLib) Max(name string) float64 {
	m := 0.0
	for i, v := range u.F.nums(name) {
		if i == 0 || v > m {
			m = v
		}
	}
...
// Min 数组特征最小值
func (u *UdfLib) Min(name string) float64 {
	m := 0.0
	for i, v := range u.F.nums(name) {
		if i == 0 || v < m {
			m = v
...
// NodeResult 单个节点的执行结果 —— 对应 repo 的 NodeResult:
// Score=节点内命中规则得分合计, Output=胜出策略名, IsBlock=流级短路信号。
// Output 被 GRL 的 when 引用作守卫 → 只能赋值语句写; Hits 只写不读 → 方法追加。
...
// Hit 追加节点内命中记录(只写不读, GRL 方法调用安全)
func (n *NodeResult) Hit(s string) { n.Hits = append(n.Hits, s) }

// FlowCtx 一次决策流执行的上下文 —— 对应 repo 的 PipelineContext:
// 携带特征, 沿途记录轨迹(AddTrack)与各节点结果(AddNodeResult)。
type FlowCtx struct {
	F           *FeatureSet
	Track       []string
...
// FlowNode 决策流节点 —— 对应 repo 的 FlowNode:
// repo 用 NextNodeName/NextNodeKind 串接, conditional/abtest 由 NodeResult 动态路由;
// 这里 Kind 决定行为: start/end 只做流转, conditional 走 Route 函数,
// ruleset/tree/table 执行各自的 grule 知识库。
type FlowNode struct {
	Name   string
	Kind   string                // start / ruleset / conditional / tree / table / end
	Next   string                // 静态下一跳(conditional 除外)
	KB     *ast.KnowledgeLibrary // grule 节点的规则库
	KBName string
	Route  func(*FlowCtx) string // conditional 专用: 返回下一节点名
}

// DecisionFlow 决策流 —— 对应 repo core/flow.go 的 DecisionFlow:
// flowMap 注册节点, Run 从 start 逐节点推进直到 end / IsBlock 短路。
type DecisionFlow struct {
	nodes map[string]*FlowNode
	start string
}

// runGruleNode 执行一个 grule 承载的节点(ruleset/tree/table):
// 每次执行取全新 KnowledgeBase 实例(工作内存不共享, 同第一部分 evaluate 的纪律),
// 注入 F(特征)/Udf(函数库)/NR(节点结果) 三个 fact。
func runGruleNode(node *FlowNode, ctx *FlowCtx) *NodeResult {
	kb, err := node.KB.NewKnowledgeBaseInstance(node.KBName, "1.0.0")
	if err != nil {
		log.Fatalf("节点 %s 获取知识库实例失败: %v", node.Name, err)
	}
	nr := &NodeResult{}
	dctx := ast.NewDataContext()
	if err := dctx.Add("F", ctx.F); err != nil {
		log.Fatal(err)
	}
	if err := dctx.Add("Udf", &UdfLib{F: ctx.F}); err != nil {
		log.Fatal(err)
	}
	if err := dctx.Add("NR", nr); err != nil {
		log.Fatal(err)
	}
	eng := engine.NewGruleEngine()
	eng.MaxCycle = 50
	if err := eng.Execute(dctx, kb); err != nil {
		log.Fatalf("节点 %s 执行失败: %v", node.Name, err)
	}
	return nr
}

// Run 决策流主循环 —— 对应 repo flow.go:
//
//	flowNode, _ := flow.GetStartNode()
//	for gotoNext { flowNode, gotoNext = flow.parseNode(flowNode, ctx) }
//	parseNode 内: if res.IsBlock { gotoNext = false }  ← 决策流执行短路
//
// 步数上限 len(nodes)+2 防 DSL 误配成环(repo 靠加载期校验, 这里给运行期保险)。
func (fl *DecisionFlow) Run(ctx *FlowCtx) {
	cur := fl.nodes[fl.start]
	for steps := 0; cur != nil && steps <= len(fl.nodes)+2; steps++ {
		switch cur.Kind {
		case "start":
			ctx.Track = append(ctx.Track, cur.Name)
			cur = fl.nodes[cur.Next]
		case "end":
			ctx.Track = append(ctx.Track, cur.Name)
			return
		case "conditional":
			// repo conditional.go: 按分支顺序求值特征条件, 首个匹配即路由;
			// 无分支匹配报 ParseErrorNoBranchMatch(本例 Route 内给了默认分支)
			next := cur.Route(ctx)
			ctx.Track = append(ctx.Track, fmt.Sprintf("%s→%s", cur.Name, next))
			cur = fl.nodes[next]
		default: // ruleset / tree / table
			nr := runGruleNode(cur, ctx)
			ctx.NodeResults[cur.Name] = nr
			ctx.TotalScore += nr.Score
			if nr.Output != "" {
				ctx.FinalOutput = nr.Output
			}
			fmt.Printf("   节点[%s] output=%s score=%.0f hits=%v\n", cur.Name, nr.Output, nr.Score, nr.Hits)
			if nr.IsBlock { // ★ 决策流执行短路: repo 的 gotoNext = !res.IsBlock
				ctx.Blocked = true
				ctx.Track = append(ctx.Track, cur.Name+"(⛔BLOCK)")
				fmt.Printf("   ⛔ 决策流短路: 节点[%s] IsBlock=true → 下游节点全部跳过\n", cur.Name)
				return
			}
			ctx.Track = append(ctx.Track, cur.Name)
			cur = fl.nodes[cur.Next]
...
// ─────────────── 节点① 准入规则集(ruleset + block_strategy) ───────────────
// 对应 repo demo/flow_ruleset.yaml:
//
//	block_strategy: { is_block: true, hit_rule: [rule_1], operator: EQ, value: reject }
//
// repo 的阻断 = 引擎比较"命中规则/输出值"后置 IsBlock;
// grule 表达: 阻断规则自己写 NR.IsBlock=true(流级信号) + Complete()(节点内短路),
// 两级短路各管一段 —— Complete 停"本规则集剩余规则", IsBlock 停"决策流后续节点"。
const grlAdmission = `
rule AdmBlacklist "准入-黑名单命中(阻断)" salience 100 {
    when
        F.StrIn("blacklist_users", F.Str("user_id"))
    then
        LogFormat("[准入] ADM-001 黑名单命中 user=%v → 阻断", F.Str("user_id"));
        NR.Output = "reject";
        NR.IsBlock = true;
        NR.Hit("ADM-001 黑名单阻断");
        Complete();
}

// UDF 进 when 参与阻断条件 —— repo 的 udf 就是给表达式用的
rule AdmPeakTxn "准入-7天单笔峰值超限(阻断)" salience 90 {
    when
        Udf.Max("txn_amounts_7d") > 50000.0
    then
        LogFormat("[准入] ADM-002 7天单笔峰值=%v 超5万 → 阻断", Udf.Max("txn_amounts_7d"));
        NR.Output = "reject";
        NR.IsBlock = true;
        NR.Hit("ADM-002 峰值超限阻断");
        Complete();
}

rule AdmPass "准入-通过" salience 1 {
    when
        NR.Output == ""
    then
        LogFormat("[准入] 通过 user=%v", F.Str("user_id"));
        NR.Output = "pass";
        NR.Hit("ADM-000 准入通过");
}
`
...
// ─────────────── 节点③a 决策树(tree) —— 新客评估 ───────────────
// 对应 repo core/tree.go: 树 = 条件逐层路由到叶子。
// GRL 表达: 每个叶子一条规则, when = 根到叶的完整路径条件;
// 路径划分完整且互斥 → 恰好一条命中(NR.Output=="" 守卫是防重保险, 同兜底层手法)。
//
//	         ┌ income < 8000  ──────────────── 叶1: C_低额度   +30
//	age<30 ──┤
//	         └ income >= 8000 ──────────────── 叶2: B_标准额度 +15
//	         ┌ device_score>=60(缺省按60) ──── 叶3: A_高额度   +5
//	age>=30 ─┤
//	         └ device_score<60 ─────────────── 叶4: B_标准额度 +20
//
// 叶3/叶4 的 F.NumOr("device_score", 60.0): 特征缺失回退默认值,
// 对应 repo feature.go GetValue 的 (value,exists) + 默认值兜底。
const grlTree = `
rule TreeLeaf1 "树叶1: 年轻×低收入 → C" salience 40 {
    when
        NR.Output == "" && F.Num("age") < 30.0 && F.Num("income") < 8000.0
    then
        LogFormat("[决策树] 叶1命中: 年轻低收入 income=%v → C_低额度", F.Num("income"));
        NR.Score = 30.0;
        NR.Output = "C_低额度";
        NR.Hit("TREE 年轻×低收入→C");
}

rule TreeLeaf2 "树叶2: 年轻×高收入 → B" salience 30 {
    when
        NR.Output == "" && F.Num("age") < 30.0 && F.Num("income") >= 8000.0
    then
        LogFormat("[决策树] 叶2命中: 年轻高收入 income=%v → B_标准额度", F.Num("income"));
        NR.Score = 15.0;
        NR.Output = "B_标准额度";
        NR.Hit("TREE 年轻×高收入→B");
}

rule TreeLeaf3 "树叶3: 成熟×设备分达标 → A" salience 20 {
    when
        NR.Output == "" && F.Num("age") >= 30.0 && F.NumOr("device_score", 60.0) >= 60.0
    then
        LogFormat("[决策树] 叶3命中: 成熟客群 设备分=%v(缺失按60) → A_高额度", F.NumOr("device_score", 60.0));
        NR.Score = 5.0;
        NR.Output = "A_高额度";
        NR.Hit("TREE 成熟×设备分达标→A");
}

rule TreeLeaf4 "树叶4: 成熟×设备分不足 → B" salience 10 {
    when
        NR.Output == "" && F.Num("age") >= 30.0 && F.NumOr("device_score", 60.0) < 60.0
    then
        LogFormat("[决策树] 叶4命中: 成熟客群 设备分=%v 不足 → B_标准额度", F.NumOr("device_score", 60.0));
        NR.Score = 20.0;
        NR.Output = "B_标准额度";
        NR.Hit("TREE 成熟×设备分不足→B");
}
`

// ─────────────── 节点③b 交叉决策表(table/matrix) —— 老客评估 ───────────────
// 对应 repo core/matrix.go(决策矩阵/交叉决策表): 行×列交叉出单元格动作。
// GRL 表达: 一个单元格一条规则, when = 行条件 && 列条件, 四格完整互斥覆盖。
//
//	                overdue=false           overdue=true
//	grade=VIP       keep_extend  +5         manual_review +40
//	grade=NORMAL    keep         +20        reduce_credit +60
const grlTable = `
rule CellVipClean "单元格: VIP×无逾期 → 提额保持" salience 40 {
    when
        NR.Output == "" && F.Str("grade") == "VIP" && F.Flag("recent_overdue") == false
    then
        LogFormat("[决策表] VIP×无逾期 user=%v → keep_extend", F.Str("user_id"));
        NR.Score = 5.0;
        NR.Output = "keep_extend_提额保持";
        NR.Hit("TABLE VIP×无逾期→提额");
}

rule CellVipOverdue "单元格: VIP×有逾期 → 转人工" salience 30 {
    when
        NR.Output == "" && F.Str("grade") == "VIP" && F.Flag("recent_overdue") == true
    then
        LogFormat("[决策表] VIP×有逾期 user=%v → manual_review", F.Str("user_id"));
        NR.Score = 40.0;
        NR.Output = "manual_review_转人工";
        NR.Hit("TABLE VIP×有逾期→人工");
}

rule CellNormalClean "单元格: NORMAL×无逾期 → 维持" salience 20 {
    when
        NR.Output == "" && F.Str("grade") == "NORMAL" && F.Flag("recent_overdue") == false
    then
        LogFormat("[决策表] NORMAL×无逾期 user=%v → keep", F.Str("user_id"));
        NR.Score = 20.0;
        NR.Output = "keep_维持额度";
        NR.Hit("TABLE NORMAL×无逾期→维持");
}

rule CellNormalOverdue "单元格: NORMAL×有逾期 → 降额" salience 10 {
    when
        NR.Output == "" && F.Str("grade") == "NORMAL" && F.Flag("recent_overdue") == true
    then
        LogFormat("[决策表] NORMAL×有逾期 user=%v → reduce_credit", F.Str("user_id"));
        NR.Score = 60.0;
        NR.Output = "reduce_credit_降额";
        NR.Hit("TABLE NORMAL×有逾期→降额");
}
`

// demoRiskEngineFlow 组装并运行决策流:
//
//	start_1 → ruleset_准入 →(可能⛔短路)→ conditional_客群分流 ┬ 新客 → tree_新客评估  → end_1
//	                                                        └ 老客 → table_老客评估 → end_1
//
// 对应 repo demo/flow_conditional.yaml 的节点串接方式(start→…→end, 条件节点动态路由)。
func demoRiskEngineFlow(zlog *zap.Logger) {
	fmt.Println("\n═══ 第二部分: 仿 skyhackvip/risk_engine 决策流(特征/UDF/决策树/决策表/短路) ═══")

	// 各 grule 节点的规则库启动时编译一次(热更新 = 重建对应节点的 Library)
	admissionLib := buildKnowledgeLibrary("node_admission", "1.0.0", grlAdmission)
	treeLib := buildKnowledgeLibrary("node_tree", "1.0.0", grlTree)
	tableLib := buildKnowledgeLibrary("node_table", "1.0.0", grlTable)

	flow := &DecisionFlow{
		start: "start_1",
		nodes: map[string]*FlowNode{
			"start_1":    {Name: "start_1", Kind: "start", Next: "ruleset_准入"},
			"ruleset_准入": {Name: "ruleset_准入", Kind: "ruleset", Next: "cond_客群分流", KB: admissionLib, KBName: "node_admission"},
			"cond_客群分流": {Name: "cond_客群分流", Kind: "conditional", Route: func(ctx *FlowCtx) string {
				// repo conditional.go: 逐分支求值(feature.Compare + EvaluateBoolExpr),
				// 首个为真的分支决定 NextNodeName; 全不匹配 repo 会报错, 这里给默认分支
				if ctx.F.Flag("is_new_customer") {
					return "tree_新客评估"
				}
				return "table_老客评估"
			}},
			"tree_新客评估":  {Name: "tree_新客评估", Kind: "tree", Next: "end_1", KB: treeLib, KBName: "node_tree"},
			"table_老客评估": {Name: "table_老客评估", Kind: "table", Next: "end_1", KB: tableLib, KBName: "node_table"},
			"end_1":      {Name: "end_1", Kind: "end"},
		},
	}

	blacklist := []string{"u_1001", "u_2002"}
	runScenario := func(label string, feats map[string]interface{}) {
		fmt.Printf("\n━━━━━━ %s ━━━━━━\n", label)
		f := NewFeatureSet(feats)
		u := &UdfLib{F: f}
		fmt.Printf("   特征: %s\n", f.Dump())
		fmt.Printf("   UDF(txn_amounts_7d): sum=%.0f avg=%.0f max=%.0f min=%.0f\n",
			u.Sum("txn_amounts_7d"), u.Avg("txn_amounts_7d"), u.Max("txn_amounts_7d"), u.Min("txn_amounts_7d"))
		ctx := &FlowCtx{F: f, NodeResults: map[string]*NodeResult{}}
		flow.Run(ctx)
		fmt.Printf("   轨迹: %s\n", strings.Join(ctx.Track, " → "))
		fmt.Printf("   终局: %s | 总分: %.0f | 流级短路: %t\n", ctx.FinalOutput, ctx.TotalScore, ctx.Blocked)
	}

	// 场景A: 黑名单用户 → 准入节点阻断 → 决策流执行短路,
	// 分流/决策树/决策表/end 全部未执行(轨迹里看不到它们)
	runScenario("决策流场景A: 黑名单用户(规则阻断→流级短路)", map[string]interface{}{
		"user_id": "u_1001", "age": 24, "income": 9000.0, "is_new_customer": true,
		"grade": "NONE", "recent_overdue": false,
		"txn_amounts_7d": []float64{1200, 800, 4300}, "blacklist_users": blacklist,
	})

	// 场景B: 干净的新客(35岁, 未提供 device_score) → 准入通过 → 条件分流走"新客"
	// → 决策树叶3(NumOr 缺省回退 60 ≥ 60) → A_高额度
	runScenario("决策流场景B: 新客→决策树(特征缺失走默认值)", map[string]interface{}{
		"user_id": "u_3003", "age": 35, "income": 20000.0, "is_new_customer": true,
		"grade": "NONE", "recent_overdue": false,
		"txn_amounts_7d": []float64{1500, 2200, 900}, "blacklist_users": blacklist,
	})

	// 场景C: VIP 老客近期有逾期 → 条件分流走"老客" → 交叉决策表 VIP×有逾期 → 转人工
	runScenario("决策流场景C: 老客→交叉决策表(VIP×逾期)", map[string]interface{}{
		"user_id": "u_4004", "age": 41, "income": 30000.0, "is_new_customer": false,
		"grade": "VIP", "recent_overdue": true,
		"txn_amounts_7d": []float64{3000, 5000}, "blacklist_users": blacklist,
	})

	// 场景D: 老客单笔 8 万触发 UDF 阻断条件(Udf.Max > 5万) → 准入节点第二条
	// 阻断规则短路 —— 演示 UDF 参与 when 并驱动流级短路
	runScenario("决策流场景D: UDF峰值阻断(Udf.Max驱动短路)", map[string]interface{}{
		"user_id": "u_5005", "age": 52, "income": 15000.0, "is_new_customer": false,
		"grade": "NORMAL", "recent_overdue": false,
		"txn_amounts_7d": []float64{2000, 80000, 3500}, "blacklist_users": blacklist,
	})
}
...
func main() {
	...
	// ★ 第二部分: 仿 skyhackvip/risk_engine 决策流
	// (特征类型系统 / UDF / 条件分流 / 决策树 / 交叉决策表 / 决策流执行短路)
	demoRiskEngineFlow(zlog)
}
